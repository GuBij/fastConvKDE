...
#include "openField.h"
 ...
 openField::openField(const dictionary& dict, bool master_proc )
 :
 openField(dict,false, master_proc)
 {}
 
 openField::openField(const dictionary& dict, bool LangevinModel, bool master_proc )
 :
 dictionary(dict,"openField",{"z0","zref","zrefTemp","zrefTemp0","zrefWindVar"}),
 name_("openField"),
 kappa(0.387),
 g(9.81),
 z0(getfl(0)),
 ...
 noGridPointsZ(ceil(5*dict.getfl(6))),
 resZ(dict.getfl(6)/ceil(5*dict.getfl(6))),
...
*/  
 ...
 double openField::stabKernel(const double& z, const string& type, const bool& integrate)
 {
   if (integrate)
   {
     if (z > 0)
       return -4.7*z; //-6.4 (H) or -5.0 (H-M)
     ...
     {
       double sqrt_x = sqrt(1-15*z), x = sqrt(sqrt_x);
     ...
     {
	cout << "\ntype " << type << " in function 'openField::stabKernel ( cont double&, const string&, const bool& )' is not implemented.\n" << endl;
	exit(EXIT_FAILURE);
     }
   }
   else
